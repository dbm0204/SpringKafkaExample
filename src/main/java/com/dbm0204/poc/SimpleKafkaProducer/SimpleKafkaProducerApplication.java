package com.dbm0204.poc.SimpleKafkaProducer;
import com.dbm0204.poc.SimpleKafkaProducer.kafkaConsumers.SimpleKafkaConsumer;
...
public class SimpleKafkaProducerApplication implements CommandLineRunner {
	...
	public void run(String... args) {
		...
		KafkaProducer<String, String> producer = 
        	...
        	/*
		 * The sendTestMessagesToKafka method will generate 
		 * some random test messag and send them to Kafka.
         	*/
        	sendTestMessagesToKafka(producer);

        	/*Now that we've produced some test messages, 
		 * let's see how to consume them using a Kafka consumer object.
         	*/
		...
		Thread kafkaConsumerThread = new Thread(() -> {
			...
			SimpleKafkaConsumer simpleKafkaConsumer = 
				new SimpleKafkaConsumer(theTechCheckTopicName,
						consumerProperties);
	...
	/*
    	...
    	private void sendTestMessagesToKafka(
			KafkaProducer<String, String> producer) {
       /*
        Creating a loop which iterates 20 times, 
	from 0 to 9, and creates an instance of JSONObject
        in each iteration. We'll use this simple JSON object 
	to illustrate how we can send a JSON
        object as a message in Kafka.
         */
        for (int index = 0; index < 20; index++) {
            /*We'll create a JSON object which will have a bunch of 
	    fields, and another JSON object,which will be nested 
	    inside the first JSON object. This is just to demonstrate how
            ...
            try {
                ...
                nestedJsonObject.put("nestedObjectMessage", 
				"This is a nested JSON object with index: " + index);
	     ...
	     * and send it to the same topic in Kafka,
	     * using the same function we used earlier.
	     * You can use any JSON library for this, 
	     * just make sure it serializes your objects properly.
